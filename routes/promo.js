...
exports.index = function(req, res) {
  Promo.where('title.lg').equals(req.locale).where('status').ne('hidden').limit(12).sort('-date').exec(function(err, promo) {
...
exports.get_promo = function(req, res) {
  ...
  Promo.where('title.lg').equals(req.locale).where('status').ne('hidden').sort('-date').skip(post.skip).limit(post.limit).exec(function(err, promo) {
    if (promo.length > 0) {
